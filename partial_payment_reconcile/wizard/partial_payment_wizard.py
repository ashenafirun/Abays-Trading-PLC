...
class PartialPaymentWizard(models.TransientModel):
    ...
    def _compute_available_lines(self):
        ...
        the invoice, with the opposite sign. Kept to same partner on purpose
        (cross-partner reconcile would corrupt the sub-ledgers). Currency is
        validated at apply time, not filtered here, so single-currency lines
        that store currency differently still show."""
        for wiz in self:
            ...
            # opposite sign: to pay a receivable (debit) we need a credit line
            if invoice_is_debit:
    ...
    def action_apply(self):
        ...
        # A line whose currency_id is the company currency (or unset) is fine.
        company_cur = self.company_currency_id
        for line in (inv_line, cp_line):
            line_cur = line.currency_id
            if line_cur and line_cur != company_cur:
                raise UserError(_(
        ...
        # Create the partial reconcile for exactly the typed amount.
        # In single currency the currency amounts equal the company amount,
        # but Odoo still needs debit_amount_currency / credit_amount_currency
        # populated or _compute_amount_residual hits a None and crashes.
