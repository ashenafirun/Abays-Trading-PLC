...
def remove_background(b64_data):
    """Remove light/beige background from signature and return transparent PNG b64."""
...
def get_sr_no(approval_request):
    """Try all possible field names for SR. NO on approval.request."""
    if not approval_request:
        return ''
    for fname in ['x_studio_sr_no', 'x_sr_no', 'x_studio_field_sr_no', 'x_name_sr']:
        try:
            val = getattr(approval_request, fname, None)
            if val:
                return val
        except Exception:
            pass
...
class StockPicking(models.Model):
    ...
    # Store Req No and Purchase Req No pulled from approval request
    x_store_req_no = fields.Char(string='Store Req. No', copy=False)
    x_purchase_req_no = fields.Char(string='Purchase Req. No', copy=False)
    ...
    def _get_employee_info(self):
        ...
        if employee:
            return employee.name, remove_background(employee.x_employee_signature)
        return self.env.user.name, False

    def _fetch_approval_fields(self):
        """Pull SR.NO and Approval number from the linked approval request."""
        try:
            po = getattr(self, 'purchase_id', False)
            appr_no = (po.origin if po else None) or self.origin or ''
            if not appr_no:
                return
            appr = self.env['approval.request'].search(
                [('name', '=', appr_no)], limit=1
            )
            sr_no = get_sr_no(appr) if appr else ''
            self.write({
                'x_store_req_no': sr_no,
                'x_purchase_req_no': appr_no,
            })
        except Exception:
            pass  # approval.request model may not exist in all environments
